...
class StudentStudent(models.Model):
    ...
    def _search(
        ...
        teacher_group = self.env.user.has_group("spd_school.group_school_teacher")
        parent_grp = self.env.user.has_group("spd_school.group_school_parent")
    ...
    def _compute_teacher_user(self):
        ...
        teacher = self.env.user.has_group("spd_school.group_school_teacher")
        for rec in self:
    ...
    def create(self, vals):
        ...
        if res.state == "draft":
            admission_group = self.env.ref("spd_school.group_is_admission")
            new_grp_list = [admission_group.id, emp_grp.id]
            res.user_id.write({"groups_id": [(6, 0, new_grp_list)]})
        elif res.state == "done":
            done_student = self.env.ref("spd_school.group_school_student")
    ...
    def admission_done(self):
        ...
        student_group = self.env.ref("spd_school.group_school_student")
