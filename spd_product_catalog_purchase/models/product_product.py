...
class Product(models.Model):
    ...
    purchase_catalog_quantity = fields.Float()
    ...
    def initiate_pol(self, operation, purchase_id):
        ...
        sale_object = po_object.search([("id", "=", purchase_id)])
        if operation == "add":
            po_object.pol_by_cart(operation, self, sale_object)
        elif operation == "remove":
            po_object.pol_by_cart(operation, self, sale_object)
        elif operation == "update":
            po_object.pol_by_cart(operation, self, sale_object)

    def action_set_quantity_purchase_order(self, quantity, operation=None):
        context = self._context.copy() or {}
        cart_details = self.utilizable_cart_details()
        if operation == "remove":
            self.purchase_catalog_quantity -= quantity
            if cart_details.get("total_count") != 0:
                if self.purchase_catalog_quantity == 0:
                    sale_order_id = cart_details.get("cart_data").order_id
                    self.env['purchase.order'].user_input_qty_pol(0, self.id, sale_order_id.id)
                else:
    ...
    def action_btn_remove_quantity(self):
        if self.purchase_catalog_quantity == 0:
            self.purchase_catalog_quantity = 0
            return
        return self.action_set_quantity_purchase_order(1, operation="remove")

    def action_btn_add_quantity(self):
        return self.action_set_quantity_purchase_order(1, operation="add")

    def action_remove_product_so_line(self):
        return self.action_set_quantity_purchase_order(self.purchase_catalog_quantity, operation="remove")
